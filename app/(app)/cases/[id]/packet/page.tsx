...
export default async function ReviewPacketPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="packet-page space-y-6 print:bg-white">
      ...
      <section className="packet-sheet rounded-lg border border-ledger bg-white p-6 shadow-hairline print:border-0 print:p-0 print:shadow-none">
        <header className="packet-header border-b border-ledger pb-5">
          <p className="text-xs font-semibold uppercase text-docket">Attorney review packet</p>
          <div className="mt-2 flex flex-col justify-between gap-4 md:flex-row md:items-end">
            <div>
              <h1 className="legal-heading packet-title text-3xl font-semibold text-ink">{packet.caseNumber}</h1>
        ...
        <div className="packet-grid mt-6 grid gap-5 print:grid-cols-2 lg:grid-cols-[320px_1fr]">
          <div className="packet-column space-y-5">
            <PacketSection title="Client">
              <dl className="packet-list-space space-y-3 text-sm">
            ...
            <PacketSection title="Readiness">
              <div className="packet-highlight rounded-md border border-ledger bg-paper p-3">
              ...
              <ul className="packet-list-space mt-4 space-y-3">
          ...
          <div className="packet-column space-y-5">
            <PacketSection title="Case narrative">
              ...
              {packet.overview.internalIntakeNotes ? (
                <div className="packet-highlight mt-4 rounded-md border border-ledger bg-paper p-3">
            ...
            <PacketSection title="Case summary">
              {packet.summary ? (
                <div className="packet-list-space space-y-4 text-sm leading-6">
                  <p className="packet-highlight rounded-md border border-ledger bg-paper p-4 text-ink">
            ...
            <PacketSection title="Documents">
              <div className="packet-divide divide-y divide-ledger/70">
                {packet.documents.map((document) => (
                  <div key={document.id} className="packet-row flex items-start justify-between gap-4 py-3 text-sm">
                    <div>
              ...
              {packet.followUps.length === 0 ? (
                ...
                <div className="packet-divide divide-y divide-ledger/70">
                  {packet.followUps.map((task) => (
                    <div key={task.id} className="packet-row flex items-start justify-between gap-4 py-3 text-sm">
                      <div>
            ...
            <PacketSection title="Internal notes">
              {packet.notes.length === 0 ? (
                ...
                <div className="packet-list-space space-y-3">
                  {packet.notes.map((note) => (
                    <div key={note.id} className="packet-highlight rounded-md border border-ledger bg-paper p-3 text-sm">
            ...
            <PacketSection title="Activity">
              <ol className="packet-list-space space-y-3">
                {packet.activity.slice(0, 5).map((log) => (
                  <li key={log.id} className="packet-activity-row border-l-2 border-ledger pl-4">
...
function PacketSection({ title, children }: { title: string; children: ReactNode }) {
  return (
    <Card className="packet-section break-inside-avoid print:border print:border-ledger print:bg-white print:shadow-none">
      <h2 className="packet-section-title mb-4 text-base font-semibold text-ink">{title}</h2>
      {children}
    </Card>
  );
}
...
function PacketList({ title, items }: { title: string; items: string[] }) {
  ...
  return (
    <div>
      ...
      <ul className="packet-list-space space-y-2">
        {items.map((item) => (
          <li key={item} className="packet-list-item rounded-md border border-ledger/80 bg-white px-3 py-2 text-ink">
            {item}
          </li>
        ))}
      </ul>
